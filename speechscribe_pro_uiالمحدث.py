...
class SpeechScribeUI(tk.Tk):
    ...
    def __init__(self):
        super().__init__()
        self.title("بسم الله الرحمن الرحيم  walidddhony-rgb/SPEECHTIME — Semi-Automatic Speech Transcription")
        ...
        self.setting_segment_length = 200.0
        self.setting_segment_overlap = 10.0
        self.setting_similarity_threshold = 0.85
    ...
    def _build_header(self):
        ...
        self._label(title_box, "SpeechTime", 15, self.TEXT, True, bg=self.HEADER).pack(anchor="w")
        ...
        decoder_text = "Audio decoders available" if (sf is not None or AudioSegment is not None) else "WAV active "
    ...
    def _build_audio_area(self, parent):
        ...
        self.whisper_button = self._button(controls, "📝 غير مفعل", self.start_whisper, accent=True, width=12)
    ...
    def _on_whisper_complete(self):
        ...
        self.transcript.insert("end", "SpeechTime WHISPER TRANSCRIPT\n\n", "title")
    ...
    def apply_reviewed_whisper_result(self, revised_result):
        ...
        self.transcript.delete("1.0", "end")
        self.transcript.insert("end", "SpeechTime REVIEWED WHISPER TRANSCRIPT\n\n", "title")
        self.transcript.insert(
            "end",
    ...
    def start_analysis(self):
        ...
        # حساب التداخل بالملي ثانية بناءً على النسبة المئوية
        overlap_ms_calculated = self.setting_segment_length * (self.setting_segment_overlap / 100.0)

        # استخدام الإعدادات المحفوظة بدلاً من الأرقام الثابتة
        self.analysis_engine.start_analysis(
            self.audio_info.path, 
            segment_ms=self.setting_segment_length, 
            overlap_ms=overlap_ms_calculated, 
            max_clusters=4000
        )
        self._poll_analysis()
    ...
    def listen_cluster(self):
        ...
        selection = self.cluster_tree.selection()
        if not selection:
            messagebox.showwarning("لم يتم التحديد", "الرجاء تحديد صف من جدول SOUND CLUSTERS أولاً.")
            return
        ...
        # البحث عن أول مقطع صوتي ينتمي لهذا الـ Cluster
        target_start = None
        target_end = None
        
        for idx, cid in enumerate(labels):
            if int(cid) == self.current_cluster_id:
                target_start = float(start_times[idx])
                target_end = float(end_times[idx])
                # إضافة هامش بسيط (100 ملي ثانية من البداية والنهاية) 
                # لجعل الصوت أوضح للمستخدم
                target_start = max(0.0, target_start-0.1 )
                target_end = min(self.total_seconds, target_end+0.1 )
                break

        if target_start is None:
            messagebox.showinfo("غير موجود", "لم يتم العثور على مقاطع صوتية لهذا الـ Cluster.")
            return

        self.timeline_status.config(text=f"●  Playing C-{self.current_cluster_id:03d} ...", fg=self.CYAN)

        # دالة التشغيل التي ستعمل في الخلفية
        def play_audio():
            try:
                import tempfile
                import os
                ...
                # استخراج المقطع الصوتي (pydub تتعامل بالملي ثانية)
                audio = AudioSegment.from_file(str(self.audio_info.path))
                segment = audio[int(target_start * 1000): int(target_end * 1000)]
                
                # إنشاء ملف مؤقت لحفظ المقطع
                with tempfile.NamedTemporaryFile(suffix=".wav", delete=False) as temp_audio:
                    temp_path = temp_audio.name
                
                # تصدير المقطع كملف wav
                segment.export(temp_path, format="wav")
                
                # تشغيل الصوت باستخدام مكتبة ويندوز المدمجة
                winsound.PlaySound(temp_path, winsound.SND_FILENAME)
                
                # تحديث حالة شريط المهام بعد الانتهاء
                self.timeline_status.config(text=f"●  Finished playing C-{self.current_cluster_id:03d}", fg=self.GREEN)
                
                # حذف الملف المؤقت بعد انتهاء التشغيل
                try:
                    os.remove(temp_path)
                except OSError:
                    pass
                ...
                self.timeline_status.config(text="●  Playback error", fg=self.RED)

        # تشغيل الصوت في Thread منفصل لكي لا يتوقف البرنامج أثناء الاستماع
        import threading
    ...
    def generate_text(self):
        ...
        self.transcript.delete("1.0", "end")
        self.transcript.insert("end", "SpeechTime REAL TRANSCRIPT\n\n", "title")
        ...
        for start, end, label in timed_segments:
            ...
            g+=f"{time_tag} time {label}\n\n"
        uuf=open(f'{self.audio_info.path.name}out.txt','w')
        bb='ابجدهوزحطيكلمنسعفصقرشتثخذضظغأإؤةى-'
        g1=''

        for i in range(len(g)):
            if bb.count(g[i])>0:
                g1+=g[i]
        uuf.write(g1.replace('-',' '))
        uuf.close()
        uuf=open(f'{self.audio_info.path.name}-srt.txt','w')
        uuf.write(g)
        uuf.close()
    ...
    def render_transcript(self, show_demo=False):
        self.transcript.delete("1.0", "end")
        self.transcript.insert("end", "SpeechTime TRANSCRIPTION PREVIEW\n\n", "title")
        if self.analysis_ready:
            ...
            self.transcript.insert("end", "اصبح الان مباشر يقوم بتوليد الكلاستر و التحويل.\n\n", "note")
        if not self.audio_info:
            self.transcript.insert("end", "يمكنك استخدام wav file\n\n")
    ...
    def show_settings(self):
        ...
        window.title("SpeechTime Settings")
        ...
        # تعريف متغيرات لواجهة المستخدم مربوطة بالقيم المحفوظة
        self.seg_len_var = tk.StringVar(value=str(self.setting_segment_length))
        self.overlap_var = tk.StringVar(value=str(self.setting_segment_overlap))
        self.sim_thresh_var = tk.StringVar(value=str(self.setting_similarity_threshold))

        fields = [
            ("Segment length (ms)", self.seg_len_var), 
            ("Segment overlap (%)", self.overlap_var), 
            ("Similarity threshold", self.sim_thresh_var)
        ]
        ...
        for title, var in fields:
            ...
            entry = tk.Entry(row, textvariable=var, bg=self.CANVAS, fg=self.TEXT, relief="flat", font=("Segoe UI", 10), width=25)
        ...
        row = tk.Frame(window, bg=self.PANEL)
        row.pack(fill="x", padx=24, pady=7)
        self._label(row, "Audio decoder", 10, self.MUTED).pack(side="left")
        decoder_lbl = tk.Label(row, text=decoder_status, bg=self.CANVAS, fg=self.TEXT, font=("Segoe UI", 10), width=25, anchor="w")
        decoder_lbl.pack(side="right", ipady=5)

        # دالة الحفظ
        def save_settings():
            try:
                # تحديث المتغيرات الأساسية بالقيم الجديدة
                self.setting_segment_length = float(self.seg_len_var.get())
                self.setting_segment_overlap = float(self.overlap_var.get())
                self.setting_similarity_threshold = float(self.sim_thresh_var.get())
                
                messagebox.showinfo("تم الحفظ", "تم حفظ الإعدادات بنجاح!", parent=window)
                window.destroy()
            except ValueError:
                messagebox.showerror("خطأ", "الرجاء إدخال أرقام صحيحة.", parent=window)

        # أزرار الحفظ والإلغاء
        btn_frame = tk.Frame(window, bg=self.PANEL)
        btn_frame.pack(fill="x", padx=24, pady=20)
        self._button(btn_frame, "Save", save_settings, accent=True).pack(side="right")
        self._button(btn_frame, "Cancel", window.destroy).pack(side="right", padx=(0, 10))
